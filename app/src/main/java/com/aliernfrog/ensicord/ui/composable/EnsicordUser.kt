...
fun EnsicordUser(user: User, modifier: Modifier = Modifier) {
    val avatar = GeneralUtil.getAvatarPainter(user.avatar)
    Row(modifier.padding(8.dp)) {
        Image(painter = avatar, contentDescription = "", Modifier.padding(end = 8.dp).clip(CircleShape).size(40.dp, 40.dp))
